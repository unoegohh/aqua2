...
class NewPriceAddCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        $this->setName('unoegohh:entity:newPriceAdd')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        //$row = 0;
        $category1= $prefRepo->findOneBy(array("name" => "Comet"));
        if($category1 == null){
            $category1 = new ItemCategory();
            $category1->setName("Comet");
        ...
        $iii = $i->findBy(array("id" =>27271));
        $items = $i->findBy(array("import" => "34"));
        ...
        $category = null;
        if (($handle = fopen($this->getContainer()->get('kernel')->getRootDir() . "/../web/987.csv", "r")) !== FALSE) {
